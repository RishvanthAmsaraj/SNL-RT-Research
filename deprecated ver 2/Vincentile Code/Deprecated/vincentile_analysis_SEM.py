...
Speed labels appear once at the top of each column.
No individual participant traces are shown — focus is on group-level trends.
Vincentile error bars show standard deviation across participants.
Peak RT (most commonly occurring RT) is annotated on each histogram.
"""
...
def aggregate_participants(file_list):
    ...
    for spd in SPEEDS:
        ...
        results[spd]["diff"] = {
            ...
            # SEM across participants — shows precision of the group mean
            # SEM = SD / sqrt(n), so it shrinks as more participants are added
            "vinc_sd":    np.nanstd(diff_mat, axis=0, ddof=ddof) / np.sqrt(n),
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = r"C:\Users\Rishv\Downloads\vincentile_results_SEM.png"
