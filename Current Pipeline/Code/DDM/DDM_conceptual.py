...
def draw(measure, s, p, med, color):
    ...
    fig.suptitle(f"DDM (Single Boundary, {measure.upper()}) -- {s} deg/s", fontsize=12.5, fontweight="bold", y=0.99)
    ...
    fig.tight_layout()
    base = os.path.join(HERE, f"ddm_{measure}_{s}_degs")
...
print("saved 6 conceptual DDM schematics to", HERE)
