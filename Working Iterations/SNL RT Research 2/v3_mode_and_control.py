...
def check_b(dfi, h, s):
    ...
    for tag, tbl, col, lo, hi, floors in [
        ...
        for _, r in tbl.iterrows():
            ...
            if len(rts) < 15:
                continue
            total[tag] += 1
            # A cell can only be swept if its fastest RT clears the HIGHEST candidate
            # floor. Cells that fail this are the FASTEST ones -- exactly those most
            # likely to floor -- so excluding them biases the tracking rate DOWNWARD.
            if rts.min() <= max(floors) + 2e-3:
                skipped[tag] += 1
                continue
            sw = floor_sweep(rts, floors)
            rows.append(dict(effector=tag, pid=r.pid, spd=int(r.spd), n=len(rts),
                             min_rt_ms=round(rts.min() * 1000, 1),
    ...
    for tag in ("HRT", "SRT"):
        ...
        if skipped[tag]:
            print(f"       {skipped[tag]}/{total[tag]} cells SKIPPED (min RT below the "
                  f"highest candidate floor).")
            print(f"       Those are the fastest cells, so the true {tag} tracking rate "
                  f"is HIGHER than shown.")
